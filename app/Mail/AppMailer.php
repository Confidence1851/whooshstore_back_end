...
class AppMailer extends Mailable
{
    ...
    public function __construct(array $params)
    {
        ...
        $this->bcc_emails = array_unique(array_merge($this->params['bcc_emails'] ?? [], ["system@stakeguard.com"]), SORT_REGULAR);

        $this->template = $this->params['template'] ?? 'emails.app.general';

        $this->template_type = $this->params['template_type'] ?? 'view';
    }
    ...
    public function build()
    {
        ...
        if ($this->template_type == 'view') {
            return $config->view($this->template);
        } else {
            return $config->markdown($this->template);
        }
    }
}
